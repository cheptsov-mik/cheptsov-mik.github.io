...
def _add_border_css(html: str) -> str:
    """
    Insert a <style> block that forces visible borders for all tables.
    The block is placed just after the opening <body> tag (or at the top
    of the document if <body> is missing).

    This approach works for the majority of Word tables because Word
    stores borders as 1 px solid lines. If you need per‑table styling,
    replace the generic CSS with a style‑map (see comments below).
    """
    css = """
    <style>
    ...
    # Try to insert after <head> if it exists, otherwise at the very start.
    if "<head>" in html:
        # Insert just before </head>
        html = html.replace("</head>", f"{css}\n</head>")
    else:
        # No <head> – prepend the CSS to the document.
        html = css + "\n" + html
    return html
...
def convert_docx_to_html(
    ...
    # ------------------------------------------------------------------ #
    # 3️⃣  Inject CSS for table borders
    # ------------------------------------------------------------------ #
